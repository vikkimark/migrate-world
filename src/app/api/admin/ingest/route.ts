...
export async function POST(req: Request) {
  ...
  // Supabase (use service role on server to bypass RLS)
  ...
  const sb = createClient(urlBase, serviceKey, {
  ...
  if (!title || (!url && !content)) {
    return NextResponse.json({ error: "Need title and either url or content" }, { status: 400 });
  }

  // Fetch & clean if only URL provided
  if (url && !content) {
    try {
  ...
  // Create/find source (your table requires `name` NOT NULL; set name=title)
  ...
  if (!sourceId) {
    ...
    sourceId = ins.data.id as number;
  }
